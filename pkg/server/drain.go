...
var (
	...
	drainWait = settings.RegisterDurationSetting(
		...
		"the amount of time a server waits in an unready state before proceeding with a drain "+
			...
			"whole draining process, while server.shutdown.drain_wait is to set the max"+
			"wait time for this draining phase)",
		0*time.Second,
		settings.NonNegativeDurationWithMaximum(10*time.Hour),
	).WithPublic()
...
func (s *drainServer) drainClients(
	...
	if shouldDelayDraining {
		...
		drainWaitLogMessage := fmt.Sprintf(
			"node is entering the unready state, and the /healthz?ready=1 "+
				"healthcheck endpoint will report the shutting-down status; proceeding "+
				"to the next draining phase after %s (configured by cluster setting "+
				"'server.shutdown.drain_wait')",
			drainWaitDuration,
		)
