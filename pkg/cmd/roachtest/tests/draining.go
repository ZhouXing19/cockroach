...
func registerDraining(r registry.Registry) {
	{
		...
		r.Add(registry.TestSpec{
			Name:    fmt.Sprintf("draining/early-exit-conn-wait"),
...
func testEarlyExitInConnectionWait(
	...
	healthEndpointRes, err = getNodeHealth(ctx, t, c, nodeToDrain, adminAddrs[0])
	if err != nil {
		t.Fatal(errors.Wrap(err, "cannot check node's health before draining starts"))
	}
	...
	if _, ok := healthEndpointRes["error"]; ok {
		t.Fatal(errors.New("node should be ready to receive new SQL connections " +
	...
	if !ok {
		t.Fatal(errors.New("node should be in shutting-down mode now"))
	}
	...
	if db.Stats().OpenConnections != 3 {
		t.Fatal(errors.New("number of open connections should be 3 at connection_wait."))
	}
	...
	if row.Err() != nil {
		t.Fatal(errors.New("cannot run query before the server draining SQL connections"))
	}
	if err := row.Scan(&queryRes); err != nil {
		t.Fatal(errors.New("cannot scan the result from query"))
	}
	...
	if actualDrainDuration >= float64(totalWaitDurationSec)-10 {
		t.Fatal(errors.New("the draining process didn't early exit when waiting all SQL " +
...
func getNodeHealth(
	ctx context.Context, t test.Test, c cluster.Cluster, node int, adminAddr string,
	...
	if err != nil {
		return nil, errors.New("cannot get the readiness of the node before drain starts")
	}

	var healthEndpointRes map[string]interface{}
	if err := json.Unmarshal([]byte(result.Stdout), &healthEndpointRes); err != nil {
		return nil, errors.Wrap(err, "cannot unmarshal stdout to json")
	}

	return healthEndpointRes, nil
