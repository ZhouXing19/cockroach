...
func InjectLegacyTable(
	...
	err := cftxn.CollectionFactoryTxn(ctx, s.CollectionFactory().(*descs.CollectionFactory), s.InternalExecutor().(sqlutil.InternalExecutor), s.DB(), func(ctx context.Context, txn *kv.Txn, descriptors *descs.Collection) error {
		id := table.GetID()
...
func GetTable(
	...
	// Retrieve the table.
	err := cftxn.CollectionFactoryTxn(ctx, s.CollectionFactory().(*descs.CollectionFactory), s.InternalExecutor().(sqlutil.InternalExecutor), s.DB(), func(ctx context.Context, txn *kv.Txn, descriptors *descs.Collection) (err error) {
