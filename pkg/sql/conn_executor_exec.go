...
func (ex *connExecutor) execStmtInOpenState(
	...
	defer func() {
		...
		if portal != nil && portal.isPausable() && !portal.pauseInfo.execStmtCleanup.isComplete {
			portal.pauseInfo.execStmtCleanup.isComplete = true
		}
	...
	if needFinish {
		sql := stmt.SQL
		defer func() {
			processCleanupFunc(portal, "finish instrumentation helper", func() {
	...
	// preparing the statement, and this function is idempotent, we don't need to
...
func (ex *connExecutor) execWithDistSQLEngine(
	...
	if planner.hasFlowForPausablePortal() {
		err = planner.resumeFlowForPausablePortal(ctx, recv)
	} else {
